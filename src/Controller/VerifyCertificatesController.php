...
class VerifyCertificatesController extends ControllerBase {
  ...
  if (!empty($qr_code)) {
    ...
    $form = $this->formBuilder->getForm('Drupal\om_flowsheet\Form\VerifyCertificatesForm');
    return [
      '#type' => 'container',
      'form' => $form,
    ];
  ...
  public function verifyQRCodeFromDB($qr_code) {
    ...
    $query = $connection->select('om_flowsheet_qr_code', 'oqc')
    ...
    if ($result && $result->proposal_id) {
      ...
      $query = $connection->select('om_flowsheet_proposal', 'ofp')
        ->fields('ofp')
